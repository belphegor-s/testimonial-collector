...
export default function BuilderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleSave() {
    ...
    await fetch(`/api/campaigns/${campaignId}`, {
      ...
      body: JSON.stringify({ form_schema: blocks }),
    });
    setSaving(false);
    setSaved(true);
      ...
      <div className="bg-white border-b border-zinc-200 px-5 h-12 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-5 h-5 rounded-md shrink-0 flex items-center justify-center" style={{ backgroundColor: (campaign.brand_color ?? '#18181b') + '25' }}>
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: campaign.brand_color ?? '#18181b' }} />
            </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs text-zinc-400">{blocks.length} block{blocks.length !== 1 ? 's' : ''}</span>
          ...
          <button
            ...
            className={cn('flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg border transition-all', previewOnly ? 'bg-zinc-900 text-white border-zinc-900' : 'bg-white text-zinc-500 border-zinc-200 hover:border-zinc-300')}
          >
                    ...
                    <DragOverlay>
                      {activeBlock && (
                        <div className="bg-white rounded-xl border border-zinc-200 px-3 py-2.5 shadow-xl opacity-95 flex items-center gap-2">
                          <div className="w-5 h-5 rounded-md flex items-center justify-center" style={{ backgroundColor: BLOCK_COLORS[activeBlock.type] + '15', color: BLOCK_COLORS[activeBlock.type] }}>
