...
export default async function BillingPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  if (activeOrg.plan === 'free') {
    const campaigns = await db.select({ id: schema.campaigns.id, name: schema.campaigns.name }).from(schema.campaigns).where(eq(schema.campaigns.organizationId, activeOrg.id));
    if (campaigns.length) {
      const counts = await Promise.all(
        campaigns.map(async (c) => {
          const [row] = await db.select({ value: count() }).from(schema.testimonials).where(eq(schema.testimonials.campaignId, c.id));
          return { id: c.id, name: c.name, count: row?.value ?? 0 };
        }),
      );
      usage = counts;
    }
    ...
    <div className="space-y-6">
      ...
      {justUpgraded && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl px-4 py-3 text-sm flex items-start gap-2">
          ...
          <div>
            ...
            <p className="text-emerald-700/90 text-xs mt-0.5">It can take a few seconds for your plan to activate. Refresh if you don&apos;t see &ldquo;Pro&rdquo; below yet.</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-zinc-200 rounded-xl p-5">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-2xl font-semibold text-zinc-900 capitalize">{activeOrg.plan === 'pro' ? 'Pro' : 'Free'}</p>
          ...
          <span className={activeOrg.plan === 'pro' ? 'text-[11px] font-medium px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 uppercase tracking-wide' : 'text-[11px] font-medium px-2 py-1 rounded-full bg-zinc-100 text-zinc-600 uppercase tracking-wide'}>
        ...
        {activeOrg.plan === 'free' ? (
          <div className="mt-5 grid gap-3">
            ...
            {usage.length === 0 ? (
              ...
              usage.map((u) => <UsageBar key={u.id} label={`Testimonials - ${u.name}`} current={u.count} max={FREE_TESTIMONIAL_LIMIT} />)
            )}
          </div>
        ) : null}
      </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-5">
          <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
            <div>
              ...
              <p className="text-sm text-zinc-500">Used for AI summaries and sentiment analysis. You get 100 credits per billing cycle.</p>
            </div>
      ...
      {activeOrg.plan === 'pro' && (
        <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
          ...
          {orders.length === 0 ? (
            <div className="px-5 py-6 text-sm text-zinc-500">No invoices yet. Your first one appears here after your first charge.</div>
          ) : (
              ...
              {orders.map((o) => (
                <div key={o.id} className="px-5 py-3 flex items-center justify-between gap-3 text-sm">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className={'text-[10px] font-medium px-1.5 py-0.5 rounded uppercase tracking-wide ' + (o.status === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-zinc-100 text-zinc-500')}>{o.status}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="px-5 py-3 border-t border-zinc-100 bg-zinc-50/50">
            <p className="text-xs text-zinc-500 inline-flex items-center gap-1">Download PDFs from the billing portal <ExternalLink size={11} /></p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-zinc-100">
          <PlanColumn name="Free" price="$0" cadence="forever" features={[`${FREE_CAMPAIGN_LIMIT} campaign`, `${FREE_TESTIMONIAL_LIMIT} testimonials per campaign`, 'Text + video collection', 'Embeddable widget']} current={activeOrg.plan === 'free'} />
          <PlanColumn name="Pro" price="$19" cadence="per month" features={['Unlimited campaigns + testimonials', 'Custom domains', '100 AI credits per month', 'Sentiment analytics', 'Team members']} current={activeOrg.plan === 'pro'} highlighted />
        </div>
...
function CreditPackCard({ amount, price, addonKey, popular }: { amount: number; price: string; addonKey: string; popular?: boolean }) {
  return (
    <div className={`relative rounded-xl border p-4 ${popular ? 'border-emerald-200 bg-emerald-50/50' : 'border-zinc-200 bg-white'}`}>
      {popular && <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[10px] font-medium px-2 py-0.5 rounded-full bg-emerald-600 text-white uppercase tracking-wide">Best value</span>}
      <div className="flex items-center gap-1 mb-1"><Zap size={12} className="text-emerald-600" /><p className="text-sm font-semibold text-zinc-900">{amount} credits</p></div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-xs mb-1">
        ...
        <span className={danger ? 'text-red-600 font-medium' : 'text-zinc-700 font-medium'}>{current} / {max}</span>
      </div>
...
function PlanColumn({ name, price, cadence, features, current, highlighted }: { name: string; price: string; cadence: string; features: string[]; current: boolean; highlighted?: boolean }) {
  return (
    <div className={'p-5 ' + (highlighted ? 'bg-zinc-50/50' : '')}>
      ...
      <p className="text-2xl font-semibold text-zinc-900">{price}<span className="text-sm font-normal text-zinc-400 ml-1">/ {cadence}</span></p>
