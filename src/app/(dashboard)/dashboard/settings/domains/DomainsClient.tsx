  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleAdd} className="bg-white border border-zinc-200 rounded-xl p-5">
        <p className="text-sm font-semibold text-zinc-900 mb-4">Add a domain</p>
      ...
      {domains.length === 0 ? (
        ...
        <ul className="space-y-3">
          {domains.map((d) => (
            <DomainRow key={d.id} domain={d} campaign={campaigns.find((c) => c.id === d.campaign_id)} />
          ))}
        </ul>
      )}
    </div>
...
function DomainRow({ domain, campaign }: { domain: Domain; campaign?: Campaign }) {
    ...
    <li className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between gap-3 p-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={remove}
            disabled={busy}
            className="text-xs text-red-500 hover:text-red-700 transition-colors p-1.5 rounded hover:bg-red-50"
      ...
      {showInstructions && !domain.verified_at && (
        <div className="border-t border-zinc-100 bg-zinc-50/50 p-4 text-sm">
          <p className="text-xs font-semibold text-zinc-700 uppercase tracking-wide mb-2">How to verify</p>
          <p className="text-xs text-zinc-500 mb-3">Add <strong>either</strong> of these DNS records, then click Verify.</p>
