...
export async function POST(req: Request) {
  ...
  await sendEmail({
    from: FROM_EMAIL,
    to: customerEmail,
    subject: `${session.user.email} would love your feedback`,
    html: `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#fff;">
        <div style="width:40px;height:40px;border-radius:10px;background:${campaign.brandColor}22;display:flex;align-items:center;justify-content:center;margin-bottom:24px;">
          <div style="width:16px;height:16px;border-radius:50%;background:${campaign.brandColor};"></div>
        </div>
        ...
        <a href="${collectionUrl}" style="display:inline-block;background:${campaign.brandColor};color:#fff;font-size:14px;font-weight:600;padding:12px 24px;border-radius:8px;text-decoration:none;">
