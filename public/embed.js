(function () {
  ...
  containers.forEach(async function (container) {
    ...
    var c =
      theme === 'dark'
        ? {
            ...
            cardHoverShadow: '0 12px 32px rgba(0,0,0,0.4)',
            text: '#fafafa',
            textSec: '#a1a1aa',
            textMuted: '#71717a',
            starFill: '#facc15',
    ...
    // Card base
    css +=
      '#' + uid + ' .tcw-card{background:' + c.cardBg + ';border:1px solid ' + c.cardBorder + ';border-radius:' + rad + 'px;padding:24px;box-sizing:border-box;display:flex;flex-direction:column;';
    ...
    // Carousel
    css += '#' + uid + ' .tcw-carousel{display:flex;gap:16px;padding:16px;overflow-x:auto;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:4px 0 16px;}\n';
    ...
    css +=
      '#' +
      uid +
      ' .tcw-marquee-track{display:flex;width:max-content;padding:16px 0;will-change:transform;animation:tcw-scroll-' +
      uid +
      ' ' +
      dur +
      's linear infinite' +
    ...
    css +=
      '#' +
      uid +
      ' .tcw-avatar{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;color:' +
    ...
    css +=
      '#' +
      uid +
      ' .tcw-vid-ov{position:absolute;inset:0;background:linear-gradient(0deg,' +
      c.videoOverlay +
      ' 0%,transparent 60%);display:flex;align-items:center;justify-content:center;transition:opacity .2s;}\n';
    ...
    css +=
      '#' +
      uid +
      ' .tcw-play{width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,.95);display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(0,0,0,.2);transition:transform .15s;}\n';
    ...
    css +=
      '#' +
      uid +
      ' .tcw-badge{display:flex;align-items:center;justify-content:center;gap:6px;margin-top:24px;padding:10px 16px;font-size:12px;color:' +
      c.badgeText +
    ...
    css +=
      '#' +
      uid +
      ' .tcw-nav button{width:36px;height:36px;border-radius:50%;border:1px solid ' +
      c.cardBorder +
      ';background:' +
      c.cardBg +
    ...
    function starSVG(fill) {
      return (
        '<svg class="tcw-star" viewBox="0 0 20 20" fill="' +
        fill +
    ...
    if (animation === 'marquee') {
      ...
      for (var pass = 0; pass < copies; pass++) {
        testimonials.forEach(function (t, idx) {
          track.appendChild(buildCard(t, idx));
        });
      }
      ...
      testimonials.forEach(function (t, idx) {
        wrapper.appendChild(buildCard(t, idx));
      });
      ...
      nav.innerHTML =
        '<button data-d="-1"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 19l-7-7 7-7"/></svg></button>' +
    ...
    if (animation === 'fade') {
      ...
      container.querySelectorAll('.tcw-fade-card').forEach(function (el) {
        observer.observe(el);
      });
    }
    ...
    badge.rel = 'noopener';
    badge.innerHTML =
      '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg> Powered by Testimonial Collector';
